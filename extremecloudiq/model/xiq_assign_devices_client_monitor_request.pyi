# coding: utf-8

"""
    ExtremeCloud IQ API

    ExtremeCloud IQ RESTful API for external and internal applications.  # noqa: E501

    The version of the OpenAPI document: 25.10.0-104
    Contact: support@extremenetworks.com
    Generated by: https://openapi-generator.tech
"""

from datetime import date, datetime  # noqa: F401
import decimal  # noqa: F401
import functools  # noqa: F401
import io  # noqa: F401
import re  # noqa: F401
import typing  # noqa: F401
import typing_extensions  # noqa: F401
import uuid  # noqa: F401

import frozendict  # noqa: F401

from extremecloudiq import schemas  # noqa: F401


class XiqAssignDevicesClientMonitorRequest(
    schemas.DictSchema
):
    """NOTE: This class is auto generated by OpenAPI Generator.
    Ref: https://openapi-generator.tech

    Do not edit the class manually.

    Device location assignment for multiple devices
    """


    class MetaOapg:
        required = {
            "client_monitor",
            "devices",
        }
        
        class properties:
        
            @staticmethod
            def devices() -> typing.Type['XiqDeviceFilter']:
                return XiqDeviceFilter
        
            @staticmethod
            def client_monitor() -> typing.Type['XiqDeviceClientMonitor']:
                return XiqDeviceClientMonitor
            __annotations__ = {
                "devices": devices,
                "client_monitor": client_monitor,
            }
    
    client_monitor: 'XiqDeviceClientMonitor'
    devices: 'XiqDeviceFilter'
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["devices"]) -> 'XiqDeviceFilter': ...
    
    @typing.overload
    def __getitem__(self, name: typing_extensions.Literal["client_monitor"]) -> 'XiqDeviceClientMonitor': ...
    
    @typing.overload
    def __getitem__(self, name: str) -> schemas.UnsetAnyTypeSchema: ...
    
    def __getitem__(self, name: typing.Union[typing_extensions.Literal["devices", "client_monitor", ], str]):
        # dict_instance[name] accessor
        return super().__getitem__(name)
    
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["devices"]) -> 'XiqDeviceFilter': ...
    
    @typing.overload
    def get_item_oapg(self, name: typing_extensions.Literal["client_monitor"]) -> 'XiqDeviceClientMonitor': ...
    
    @typing.overload
    def get_item_oapg(self, name: str) -> typing.Union[schemas.UnsetAnyTypeSchema, schemas.Unset]: ...
    
    def get_item_oapg(self, name: typing.Union[typing_extensions.Literal["devices", "client_monitor", ], str]):
        return super().get_item_oapg(name)
    

    def __new__(
        cls,
        *_args: typing.Union[dict, frozendict.frozendict, ],
        client_monitor: 'XiqDeviceClientMonitor',
        devices: 'XiqDeviceFilter',
        _configuration: typing.Optional[schemas.Configuration] = None,
        **kwargs: typing.Union[schemas.AnyTypeSchema, dict, frozendict.frozendict, str, date, datetime, uuid.UUID, int, float, decimal.Decimal, None, list, tuple, bytes],
    ) -> 'XiqAssignDevicesClientMonitorRequest':
        return super().__new__(
            cls,
            *_args,
            client_monitor=client_monitor,
            devices=devices,
            _configuration=_configuration,
            **kwargs,
        )

from extremecloudiq.model.xiq_device_client_monitor import XiqDeviceClientMonitor
from extremecloudiq.model.xiq_device_filter import XiqDeviceFilter
